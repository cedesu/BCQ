...
class sqil(object):
	...
	def train(self, replay_buffer_demo,replay_buffer_exp):
		q_loss_demo=self._compute_loss(replay_buffer_demo)
		q_loss_exp=self._compute_loss(replay_buffer_exp)

		Q_loss = q_loss_demo+q_loss_exp
	...
	def _compute_loss(self,replay_buffer):
		# Sample replay buffer
		state, action, next_state, reward, done = replay_buffer.sample()

		# Compute the target Q value
		with torch.no_grad():
			target_next_qout=self.target_Q(next_state)
			next_q_max=torch.broadcast_tensors(target_next_qout.max(dim=-1,keepdim=True)[0],target_next_qout)[0]
			next_q_soft=next_q_max[:,0]+(target_next_qout-next_q_max).exp().sum(dim=-1).log()

			target_Q = reward + done * self.discount * next_q_soft
