...
# Trains BCQ offline
def train_sqil(env, replay_buffer_demo,replay_buffer_exp, is_atari, num_actions, state_dim, device, args, parameters):
	# For saving files
	setting = f"{args.env}_{args.seed}"
	buffer_name = f"{args.buffer_name}_{setting}"
	...
	# Load replay buffer	
	replay_buffer_demo.load_sqil(f"./buffers/{buffer_name}")
	...
	for t in range(int(args.max_timesteps)):
		...
		# Store data in replay buffer
		replay_buffer_exp.add(state, action, next_state, reward, done_float, done, episode_start)
		state = copy.copy(next_state)
		episode_start = False

		# Train agent after collecting sufficient data
		if args.train_behavioral and t >= parameters["start_timesteps"] and (t+1) % parameters["train_freq"] == 0:
			policy.train(replay_buffer_exp)
...
if __name__ == "__main__":
	...
	replay_buffer_2 = utils.ReplayBuffer(state_dim, is_atari, atari_preprocessing, parameters["batch_size"], parameters["buffer_size"], device)

	if args.train_behavioral or args.generate_buffer:
		interact_with_environment(env, replay_buffer, is_atari, num_actions, state_dim, device, args, parameters)
	else:
		train_BCQ(env, replay_buffer,replay_buffer_2 is_atari, num_actions, state_dim, device, args, parameters)